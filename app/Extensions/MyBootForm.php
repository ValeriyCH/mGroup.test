...
class MyBootForm extends BootstrapForm
{
    /**
    ...
    public function checkbox($name, $label = null, $value = 1, $checked = null, array $options = [])
    {
        if(isset($options['wrapper'])){
            $wrapperOptions_get = $options['wrapper'];
        ...
        $wrapperOptions = array_merge($wrapperOptions, isset($wrapperOptions_get) ? $wrapperOptions_get : []);
     ...
     * @return string
     */
    public function checkboxElement($name, $label = null, $value = 1, $checked = null, $inline = false, array $options = [], $is_group = false)
    {
        $label = $this->getLabelTitle($label, $name, $is_group);

        $labelOptions = $inline ? ['class' => 'checkbox-inline'] : [];

        $inputElement = $this->form->checkbox($name, $value, $checked, $options);
        $labelElement = '<label ' . $this->html->attributes($labelOptions) . '>' . $inputElement .'<span>'. $label . '</span>' .  '</label>';
    ...
    public function checkboxes($name, $label = null, $choices = [], $checkedValues = [], $inline = false, array $options = [])
    {
        ...
        foreach ($choices as $value => $choiceLabel) {
            ...
            $elements .= $this->checkboxElement($name, $choiceLabel, $value, $checked, $inline, $options, true);
        }

        $wrapperOptions = $this->isHorizontal() ? ['class' => $this->getRightColumnClass()] : [];
        $wrapperOptions['id'] = $name;
    ...
    {
        $label = $this->getLabelTitle($label, $name, true);
    ...
    public function input($type, $name, $label = null, $value = null, array $options = [])
    {
        ...
        $controlOptions = [];
        if(isset($options['wrapper'])){
            $controlOptions = $options['wrapper'];
            unset($options['wrapper']);
        }
        ...
        return $this->getFormGroupWithLabel($name, $label, $wrapperElement, $controlOptions);
    }
     ...
     * @return string
     */
    protected function getLabelTitle($label, $name, $isgroup_item = false)
    {
        $model = $this->form->getModel();
        if(method_exists($model, 'getAttributeLabel') && !$isgroup_item)
            if(($label = $model->getAttributeLabel($name)) != $name)
                return $label;
     ...
     * @return string
     */
    protected function getFormGroupWithLabel($name, $value, $element, $controlOptions = [])
    {
        $options = $this->getFormGroupOptions($name, $controlOptions);
    ...
    protected function getFieldError($field, $format = '<span class="help-block">:message</span>')
    {
        ...
        if ($this->getErrors()) {
            ...
            return "<span id='{$name}_error' class='error-block help-block' style='display: none'></span>";
        }
    }

    /**
     * Generate the id model_attribute
     * @param $model
     * @param $attribute
     * @return string
     */
    public function getActiveId($model, $attribute){
        $attributes = $model->attributesLabels();
        return array_key_exists($attribute, $attributes) ? class_basename($model).'_'.$attribute : $attribute;
    }
